...
impl Executor {
        ...
        for stmt in stmts.iter() {
            match stmt {
                Stmt::VarDecl(s) => {
                    ...
                    self.define(&s.token.lexeme, value)
                        .expect("Defining failed");
                }
                Stmt::Return(s) => {
    ...
    pub fn evaluate(&self, expr: &Expr) -> VarValue {
        let evaluator = ExprEvaluator { values: &self.env };
    ...
    pub fn define(&mut self, name: &str, value: VarValue) -> Result<(), VarEnvError> {
        if self.env.contains_key(name) {
            return Err(VarEnvError::new(
                name,
                "Attempted to initialize an exisiting variable.",
            ));
        }
    ...
    pub fn assign(&mut self, name: &str, value: VarValue) -> Result<(), VarEnvError> {
        ...
        Err(VarEnvError::new(
            name,
            "Attempted to assign an unknown variable.",
        ))
    }
}
